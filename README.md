[![PyPI](https://img.shields.io/pypi/v/napalm-exos.svg)](https://pypi.python.org/pypi/napalm-exos)
...
call it `EXOS` so everytime you see `EXOS` on this repo, replace it with the name you just
picked.
...
1. Replace the occurrences on this file of `EXOS`. Replace only the ones above
...
All the code should be inside `napalm_exos`. You are free to organize your code as you want,
however, there are some parts that have to be done in a certain way:

* `napalm_exos/__init__.py` - That file should import your class driver. That's what the
dynamic importer will expect.
* `napalm_exos/exos.py` - Here goes your driver.
* `napalm_exos/templates/` - We use this folder to store templates used by the `load_template`
method.
* `napalm_exos/utils/` - For consistency with other repos we recommend putting your additional
code here. Helper functions or anything you want to keep out of the main driver file.
* `napalm_exos/utils/textfsm_templates` - For consistency as well, we recommend keeping your
...
* `test/unit/exos/` - Here goes some configuration files that are used by `TestConfigDriver`.
* `test/unit/exos/mock_data/` - Here goes files that contain mocked data used by
                                    `TestGetterDriver`.
